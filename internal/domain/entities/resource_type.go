...
type ResourceType struct {
	...
	ParentResourceTypeID *uuid.UUID `gorm:"type:uuid;index:idx_resources_parent_resource_id"` // Nullable, self-referencing
	Icon                 *string    `gorm:"type:varchar(255)"`                                // Nullable

	// Self-referencing relationships
	ParentCategory *ResourceType  `gorm:"foreignKey:ParentCategoryID;constraint:OnUpdate:CASCADE,OnDelete:SET NULL"`
	SubCategories  []ResourceType `gorm:"foreignKey:ParentCategoryID"`
